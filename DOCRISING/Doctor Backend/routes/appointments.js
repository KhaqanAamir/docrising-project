...
router.post('/getAppointments',  async (req, res) => {
    ...
    try {
      const app = await Appointments.find({doc_id: req.body.docid, Status: "Pending"})
      res.send(app)
    } catch (error) {
  ...
  router.post('/getAppAppointments',  async (req, res) => {
    ...
    try {
      const app = await Appointments.find({doc_id: req.body.docid, Status: "Approved"})
      res.send(app)
    } catch (error) {
      console.error(error.message);
      res.status(500).send("Internal Server Error");
  ...
  router.put('/updateapp/:id', async (req, res) => {
    try {
       ...
       const user = await User.findByIdAndUpdate(req.userid, {$push: newUser}, {new: true});
